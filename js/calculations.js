...
const CalcEngine = {
  processRepData(rep, generalRules, groupRules) {
    ...
    let blockers = [];
    if (isGenTargetMandatory && !passGate_GenTarget) blockers.push(`باقي للهدف العام ${Math.round(remainingGenSales).toLocaleString()} ر.س`);
    if (!passGate_MandatoryGroups) blockers.push(`مجموعات إلزامية غير محققة: [${failedMandatoryGroups.join('، ')}]`);
    if (assignedGroupsCount > 0 && !passGate_MinGroupsCount) blockers.push(`حقق ${qualifiedGroupsCount} من أصل ${effectiveMinGroupsReq} مجموعات مطلوبة`);
    ...
    return {
      ...
      detailedGroups: detailedGroups.map(grp => ({ ...grp, commEarned: isEligibleForGroupCommissions ? grp.potentialComm : 0 })),
      totalGroupCommissionEarned,
      grandTotalCommission,
      eligibilityStatusText: blockers.length === 0 ? 'مستحق بالكامل ✅' : `محجوبة: ${blockers.join(' | ')}`
    };
  },
  ...
  calculateCompanyTotals(processedReps) {
    ...
    return {
      genTarget,
      genSales,
      overallGenPct: genTarget > 0 ? (genSales / genTarget) * 100 : 0,
      remainingGenSalesTotal: Math.max(0, genTarget - genSales),
      repGroupsSalesTotal,
      unmappedSalesTotal: Math.max(0, genSales - repGroupsSalesTotal),
      coveragePct: genSales > 0 ? (repGroupsSalesTotal / genSales) * 100 : 0,
      groupCommSum,
      genTargetCommSum,
      grandComm,
      qualifiedCount,
